...
function App() {
  ...
  const [query, setQuery] = useState({
    query: `{ launches(limit: 50, sort: "launch_date_utc", order: "desc") { id launch_date_utc launch_year mission_name links { mission_patch flickr_images } rocket { rocket_name } ships { id image name year_built } } } `,
    variables: {},
  });
              ...
              <div class="p-2 max-w-2xl">
                {item?.ships?.length > 1 ? (
                  ...
                  item?.ships?.map((ship_image) => (
                    <div>
                      <img src={ship_image?.image} alt="rocket" />
                      <p className="legend" class="text-white">{`${
                        ship_image?.name
                      } ${
                        ship_image?.year_built
                          ? `(${ship_image?.year_built})`
